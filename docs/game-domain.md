...
### Buildings
currently, buildings do nothing except act as a blocking/defense unit. However, in the future, buildings might be able to spawn new unit types, based on the type of building.  Additionally, buildings might be able to generate money at the beginning of a player's term, giving them a form of income. Consider that a "bank" might generate more income than a "house", though both might not be able to spawn any units. An airport might be able to spawn pilots, or airplane vehicles. A zoo might be able to spawn animals. These definitions would need to be added to 'common'. 
...
## Money 
Introducing money as a game attribute for each player would open up the ability to let players build buildings, which in turn could create income added on every turn, allowing players to spawn new units, and expand their power.
