...
The student athlete model case (`tests/e2e/student-athlete-model.spec.js`) checks mesh selection, attack-menu access, camera rotation, and renderer switching for both teams beside a generic worker. Front and rotated screenshots cover the graduation cap, tassel, basketball, jersey, and team colors. Its manifest test checks dedicated routing, cap clearance, and generic person fallbacks.
