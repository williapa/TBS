...
The standalone local-game checks can run without Supabase: after `pnpm build`, run `pnpm exec playwright test tests/e2e/solo-mode.spec.js tests/e2e/capital-model.spec.js tests/e2e/zoo-model.spec.js tests/e2e/port-model.spec.js tests/e2e/soldier-model.spec.js tests/e2e/dragon-model.spec.js tests/e2e/lion-model.spec.js tests/e2e/scientist-model.spec.js tests/e2e/blues-musician-model.spec.js tests/e2e/missile-model.spec.js tests/e2e/nuke-model.spec.js tests/e2e/money-model.spec.js`. The model cases check mesh selection and action menus before and after camera rotation, renderer switching, and browser errors; they attach front and rotated screenshots showing both team colors alongside generic building/person fallbacks. The port case shows both teams on water, with dock, crane, and container silhouettes visible from two camera angles. The zoo, port, soldier, dragon, lion, scientist, and blues musician manifest tests also check dedicated routing and health-bar clearance above the zoo giraffe and entrance, port crane, helmet, raised wings, mane, scientist hair and flask, or musician fedora and guitar. The dragon and lion browser cases include both team colors alongside the other distinct animal model. The missile, nuke, and moneybag cases check neutral-object mesh selection after camera rotation and renderer switching, and attach front and rotated views alongside other neutral objects.
