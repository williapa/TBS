...
- `pnpm nx run @TBS/renderer-3d:test`: axial world projection, instanced-terrain lookup, terrain-safe action and selection border layers with explicit selection render priority, proportional health-bar coverage anchored to the track's left edge, bounded camera transitions, distinct capital/leader asset routing and health-bar heights, procedural asset fallbacks, and deterministic movement interpolation/reduced-motion settling.
