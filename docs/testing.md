...
The standalone local-game checks can run without Supabase: after `pnpm build`, run `pnpm exec playwright test tests/e2e/solo-mode.spec.js tests/e2e/capital-model.spec.js tests/e2e/soldier-model.spec.js tests/e2e/dragon-model.spec.js`. The model cases check mesh selection and action menus before and after camera rotation, renderer switching, and browser errors; they attach front and rotated screenshots showing both team colors alongside generic building/person fallbacks. The soldier and dragon manifest tests also check dedicated routing and health-bar clearance above the helmet or raised wings. The dragon browser case includes both team colors and an animal fallback.
