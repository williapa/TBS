...
export class SupabaseIdentityAdapter implements IdentityPort {
  ...
  private async initialize(): Promise<GameSessionIdentity> {
    ...
    if (existing.error) throw unavailable("could not restore the browser session", existing.error);
    if (existing.data.session?.user.id) return { userId: existing.data.session.user.id };
...
export { SupabaseIdentityAdapter as SupabaseAnonymousIdentityProvider };
