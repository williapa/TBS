...
describe("renderer-3d presentation behavior", () => {
  ...
  it("resolves the lab with health clearance above its rooftop flask", () => {
  ...
  it("resolves the leader's distinct model without changing other people or unknown assets", () => {
    ...
    for (const assetId of ["unit:warrior", "unit:future-leader"]) {
  ...
  it("resolves the construction worker's distinct model without changing other workers or people", () => {
    ...
    for (const assetId of ["unit:warrior", "unit:future-constructionWorker"]) {
      expect(getProceduralModel(assetId).kind).toBe("person");
    }
  });

  it("resolves workers with clearance above the mechanic cap and distinct trade models", () => {
    for (const assetId of ["unit:worker", "worker"]) {
      expect(getProceduralModel(assetId)).toEqual({ assetId, kind: "worker", healthBarHeight: 1.42, source: "project-owned-procedural" });
      expect(getProceduralModel(assetId).healthBarHeight).toBeGreaterThan(1.302);
    }
    expect(getProceduralModel("unit:engineer").kind).toBe("engineer");
    expect(getProceduralModel("unit:constructionWorker").kind).toBe("construction-worker");
    expect(getProceduralModel("unit:future-worker").kind).toBe("person");
  });
  ...
  it("resolves the soldier's distinct model with helmet clearance while retaining person fallbacks", () => {
    ...
    for (const assetId of ["unit:warrior", "unit:future-soldier"]) {
      expect(getProceduralModel(assetId).kind).toBe("person");
    }
  });
  ...
  it("resolves the scientist's distinct model with hair and flask clearance while retaining person fallbacks", () => {
    ...
    for (const assetId of ["unit:warrior", "unit:future-scientist"]) {
      expect(getProceduralModel(assetId).kind).toBe("person");
    }
  });
