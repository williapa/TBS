...
test("the church remains selectable through its 3D model and after camera rotation", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "church-model", name: "Church model", map: [
      [cell(0, 0, 0, [1, 2], "soldier", "purple"), cell(0, 1, 1, [0, 2, 3], "soldier", "orange")],
      [cell(1, 0, 2, [0, 1, 3, 5], "church", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "church", "purple"), cell(1, 2, 4, [3, 6], "house", "purple")],
  ...
  await page.getByRole("option", { name: /Church model/ }).click();
  ...
  await expect(async () => {
    await clickChurch();
    await expect(page.locator(".game-panel__value").getByText("Church", { exact: true })).toBeVisible();
  }).toPass();
  await clickChurch();
  ...
  await testInfo.attach("church-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("church-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
  await testInfo.attach("church-model-front", { body: await board.screenshot({ path: testInfo.outputPath("church-model-front.png") }), contentType: "image/png" });
  await page.getByRole("button", { name: "Use 2D board" }).click();
  await expect(page.getByRole("button", { name: /Church, purple team/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /Church, orange team/ })).toBeVisible();
  await page.getByRole("button", { name: "Use 3D board" }).click();
  await expect(canvas).toBeVisible();
  await expect(async () => {
    await clickChurch();
