...
test("doctors remain selectable through their 3D models across camera and renderer changes", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "doctor-model", name: "Doctor model", map: [
      [cell(0, 0, 0, [1, 2], "none", "gray"), cell(0, 1, 1, [0, 2, 3], "none", "gray")],
      [cell(1, 0, 2, [0, 1, 3, 5], "doctor", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "doctor", "purple"), cell(1, 2, 4, [3, 6], "scientist", "purple")],
  ...
  await page.getByRole("option", { name: /Doctor model/ }).click();
  ...
  await expect(async () => {
    await clickDoctor();
    await expect(page.locator(".game-panel__value").getByText("Doctor", { exact: true })).toBeVisible();
  }).toPass();
  await clickDoctor();
  ...
  await testInfo.attach("doctor-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("doctor-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
  await testInfo.attach("doctor-model-front", { body: await board.screenshot({ path: testInfo.outputPath("doctor-model-front.png") }), contentType: "image/png" });
  await page.getByRole("button", { name: "Use 2D board" }).click();
  await expect(page.getByRole("button", { name: /Doctor, purple team/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /Doctor, orange team/ })).toBeVisible();
