...
const cell = (row, column, index, neighbors, unit, team) => ({
  row, column, index, neighbors, terrain: unit === "port" || unit === "none" ? "water" : "plains", unit, team,
});

test("the port remains selectable through its 3D model and after camera rotation", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "port-model", name: "Port model", map: [
      [cell(0, 0, 0, [1, 2], "soldier", "purple"), cell(0, 1, 1, [0, 2, 3], "soldier", "orange")],
      [cell(1, 0, 2, [0, 1, 3, 5], "port", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "port", "purple"), cell(1, 2, 4, [3, 6], "bank", "purple")],
  ...
  await page.getByRole("option", { name: /Port model/ }).click();
  ...
  await expect(async () => {
    await clickPort();
    await expect(page.locator(".game-panel__value").getByText("Port", { exact: true })).toBeVisible();
  }).toPass();
  await clickPort();
  ...
  await testInfo.attach("port-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("port-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
  await testInfo.attach("port-model-front", { body: await board.screenshot({ path: testInfo.outputPath("port-model-front.png") }), contentType: "image/png" });
  await page.getByRole("button", { name: "2D Board" }).click();
  await expect(page.getByRole("button", { name: /Port, purple team/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /Port, orange team/ })).toBeVisible();
