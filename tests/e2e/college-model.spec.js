...
test("the college remains selectable through its 3D model and after camera rotation", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "college-model", name: "College model", map: [
      [cell(0, 0, 0, [1, 2], "soldier", "purple"), cell(0, 1, 1, [0, 2, 3], "soldier", "orange")],
      [cell(1, 0, 2, [0, 1, 3, 5], "college", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "college", "purple"), cell(1, 2, 4, [3, 6], "capital", "orange")],
  ...
  await page.getByRole("option", { name: /College model/ }).click();
  ...
  await expect(async () => {
    await clickCollege();
    await expect(page.locator(".game-panel__value").getByText("College", { exact: true })).toBeVisible();
  }).toPass();
  await clickCollege();
  ...
  await testInfo.attach("college-model-rotated", { body: await board.screenshot(), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
  await testInfo.attach("college-model-front", { body: await board.screenshot(), contentType: "image/png" });
  await page.getByRole("button", { name: "2D Board" }).click();
  await expect(page.getByRole("button", { name: /College, purple team/ })).toBeVisible();
