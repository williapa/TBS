...
test("blues musicians remain selectable through their 3D models across camera and renderer changes", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "blues-musician-model", name: "Blues musician model", map: [
      [cell(0, 0, 0, [1, 2], "none", "gray"), cell(0, 1, 1, [0, 2, 3], "none", "gray")],
      [cell(1, 0, 2, [0, 1, 3, 5], "bluesMusician", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "bluesMusician", "purple"), cell(1, 2, 4, [3, 6], "priest", "purple")],
  ...
  await page.getByRole("option", { name: /Blues musician model/ }).click();
  ...
  await expect(async () => {
    await clickMusician();
    await expect(page.locator(".game-panel__value").getByText("Blues Musician", { exact: true })).toBeVisible();
  }).toPass();
  await clickMusician();
  ...
  await testInfo.attach("blues-musician-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("blues-musician-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
  await testInfo.attach("blues-musician-model-front", { body: await board.screenshot({ path: testInfo.outputPath("blues-musician-model-front.png") }), contentType: "image/png" });
  await page.getByRole("button", { name: "2D Board" }).click();
  await expect(page.getByRole("button", { name: /Blues Musician, purple team/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /Blues Musician, orange team/ })).toBeVisible();
