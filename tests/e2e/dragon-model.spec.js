...
test("dragons remain selectable through their 3D models across camera and renderer changes", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "dragon-model", name: "Dragon model", map: [
      [cell(0, 0, 0, [1, 2], "none", "gray"), cell(0, 1, 1, [0, 2, 3], "none", "gray")],
      [cell(1, 0, 2, [0, 1, 3, 5], "dragon", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "dragon", "purple"), cell(1, 2, 4, [3, 6], "lion", "purple")],
  ...
  await page.getByRole("option", { name: /Dragon model/ }).click();
  ...
  await expect(async () => {
    await clickDragon();
    await expect(page.locator(".game-panel__value").getByText("Dragon", { exact: true })).toBeVisible();
  }).toPass();
  await clickDragon();
  ...
  await testInfo.attach("dragon-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("dragon-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
