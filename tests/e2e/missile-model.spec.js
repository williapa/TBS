...
test("the neutral missile remains selectable across camera and renderer changes", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "missile-model", name: "Missile model", map: [
      [cell(0, 0, 0, [1, 2], "soldier", "purple"), cell(0, 1, 1, [0, 2, 3], "soldier", "orange")],
      [cell(1, 0, 2, [0, 1, 3, 5], "nuke", "gray"), cell(1, 1, 3, [1, 2, 4, 5, 6], "missile", "gray"), cell(1, 2, 4, [3, 6], "money", "gray")],
  ...
  await page.getByRole("option", { name: /Missile model/ }).click();
  ...
  const selectMissile = async () => {
    // Select another entity first so an old missile selection cannot mask a missed mesh hit.
    await page.getByRole("button", { name: "Select Nuke, neutral", exact: true }).click();
  ...
  await testInfo.attach("missile-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("missile-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
