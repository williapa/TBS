...
test("soldiers remain selectable through their 3D models across camera and renderer changes", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "soldier-model", name: "Soldier model", map: [
      [cell(0, 0, 0, [1, 2], "none", "gray"), cell(0, 1, 1, [0, 2, 3], "none", "gray")],
      [cell(1, 0, 2, [0, 1, 3, 5], "soldier", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "soldier", "purple"), cell(1, 2, 4, [3, 6], "priest", "purple")],
  ...
  await page.getByRole("option", { name: /Soldier model/ }).click();
  ...
  await expect(async () => {
    await clickSoldier();
    await expect(page.locator(".game-panel__value").getByText("Soldier", { exact: true })).toBeVisible();
  }).toPass();
  await clickSoldier();
  ...
  await testInfo.attach("soldier-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("soldier-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
