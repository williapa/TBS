...
test("the house remains selectable through its 3D model and after camera rotation", async ({ page }, testInfo) => {
  const errors = [];
  page.on("pageerror", (error) => errors.push(error.message));
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.addInitScript((maps) => localStorage.setItem("TBS.maps.v1", JSON.stringify(maps)), {
    repositoryVersion: 1,
    maps: [{ schemaVersion: 1, id: "house-model", name: "House model", map: [
      [cell(0, 0, 0, [1, 2], "soldier", "purple"), cell(0, 1, 1, [0, 2, 3], "soldier", "orange")],
      [cell(1, 0, 2, [0, 1, 3, 5], "house", "orange"), cell(1, 1, 3, [1, 2, 4, 5, 6], "house", "purple"), cell(1, 2, 4, [3, 6], "church", "purple")],
  ...
  const clickHouse = async () => {
    // Clear selection so a previous hit cannot hide a missed house mesh.
    await page.keyboard.press("Escape");
    await expect(page.locator(".game-panel__value").getByText("House", { exact: true })).toHaveCount(0);
  ...
  await expect(async () => {
    await clickHouse();
    await expect(page.locator(".game-panel__value").getByText("House", { exact: true })).toBeVisible();
  }).toPass();
  await clickHouse();
  ...
  await testInfo.attach("house-model-rotated", { body: await board.screenshot({ path: testInfo.outputPath("house-model-rotated.png") }), contentType: "image/png" });
  for (let index = 0; index < 5; index += 1) {
    await page.getByRole("button", { name: "Rotate camera clockwise" }).click();
  }
