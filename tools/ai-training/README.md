...
The exporter verifies live preset/protocol provenance, checkpoint and qualification hashes, PyTorch invariance/equivariance, dynamic shapes, and native ONNX parity. Run `onnx-verify` and `onnx-smoke` against the generated files to verify Web/WASM parity and an engine-applied Four Forests action. The generated bundle remains conservatively marked `qualifiedForPlayerRelease: false` because the exporter cannot attest a host application. The player application closes that separate integration gate by pairing the qualified artifact with the matching versioned postprocessor, worker runtime, resource bounds, and failure handling.
