import { applyGameAction, AppliedAction, GameSnapshot } from "@TBS/common";
import { GameRevisionNotice, GameSessionGateway, PresenceState, Unsubscribe } from "./GameSessionGateway";
import { MAX_REPLAY_GAP } from "../productLimits";

type ReconciliationGateway = Pick<GameSessionGateway, "getActions" | "getSnapshot" | "subscribe">;
...
export type GameRevisionReconcilerOptions = {
  maxReplayGap?: number;
...
export class GameRevisionReconciler {
  ...
  private readonly onPresence?: (presence: PresenceState[]) => void;
  ...
  constructor(private readonly gateway: ReconciliationGateway, options: GameRevisionReconcilerOptions = {}) {
  ...
  async start(gameId: string, listener: ReconciliationListener): Promise<Unsubscribe> {
    ...
    const initial = await this.gateway.getSnapshot(gameId);
    ...
    this.unsubscribe = await this.gateway.subscribe(gameId, (notice) => {
    ...
    // Close the snapshot/subscription race even when its Broadcast was emitted
    // before the private channel reached SUBSCRIBED.
  ...
  private enqueue(work: () => Promise<void>) {
    ...
    this.queue = next.catch((error) => { this.onError?.(error); });
    return next;
  }
  ...
  private async reconcileFromHistory(targetRevision: number | undefined, generation: number) {
    ...
    try {
      actions = await this.gateway.getActions(gameId, current.state.revision);
    } catch {
      await this.fallbackToSnapshot(generation);
      return;
    }
  ...
  private async fallbackToSnapshot(generation: number) {
    const gameId = this.gameId;
    if (!gameId) return;
    const snapshot = await this.gateway.getSnapshot(gameId);
    if (generation !== this.generation) return;
